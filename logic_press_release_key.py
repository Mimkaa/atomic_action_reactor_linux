...
def find_runner_file(ctx) -> tuple[Path | None, Path | None]:
    """
    Returns:
      (runner_file, action_dir)

    Searches both:
      - ctx.runner_file / ctx.action_dir if already correct
      - runtime/keyActions/<action>/run_RunClass.py
      - sibling locations around ctx.runner_file
    """
    action_name = (os.environ.get("ACTION_NAME", "") or "").strip()
    candidates: list[tuple[Path, Path]] = []

    # 1. trust ctx first if valid
    try:
    ...
    # 5. downloaded_test_runtime variants
    if action_name:
        candidates.append(
